...
Obfuscation and fraud of business ownership is a serious problem for governments and financial institutions when combatting tax evasion and financial fraud. This study investigates the use of graph neural networks (GNNs) for the task of detecting anomalous business relationships in a dataset of company ownership information. The dataset was generated by simulating anomalous ownership structures by introducing random perturbations to the edges of otherwise genuine nodes. The task was to identify the resulting anomalous nodes in a highly imbalanced dataset. The GNN models studied are k-GNN [@morrisWeisfeilerLemanGo2021] and GraphSAGE [@hamiltonInductiveRepresentationLearning2018]. Both GNN models were found to significantly outperform the benchmark gradient boosted tree ensemble, with k-GNN comfortably outperforming GraphSAGE on this task. The results of this study suggest that GNNs are a promising approach for the task of detecting anomalous business relationships. We propose a set of recommendations for practitioners interested in applying GNNs to fraud detection tasks, and discuss avenues for further work.
